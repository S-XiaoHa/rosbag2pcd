...
def main():
    ...
    parser.add_argument('--path', type=str, required=True, default="",help='场景路径，例如 scene_train1-20251022021524')
    ...
    print(f"开始检查 {label_dir} 下的标注文件...\n")

    # 统计总帧数
    txt_files = [f for f in os.listdir(label_dir) if f.endswith('.txt')]
    total_frames = len(txt_files)
    if total_frames == 0:
        print(f"[警告] 在 {label_dir} 下未发现任何 .txt 文件")
        return
