...
LIDAR_MAP = {
    ...
    "LIDAR_MID": "LIDAR_MID",       # 主雷达
    "LIDAR_LEFT": "LIDAR_LEFT",  # 前雷达
    "LIDAR_RIGHT": "LIDAR_RIGHT"     # 后雷达 
}
# ----------------------------------------
...
def save_pcd_with_intensity(points, file_path):
    """
    保存为 Binary PCD，包含 intensity (x, y, z, i)
    """
    num_points = len(points)
    
    # 构建 Header，注意最后一行改为 DATA binary
    # 所有的字段类型设为 F (Float 4字节)，这与 numpy.float32 对应
...
"""
    ...
    with open(file_path, 'wb') as f:
        # 1. 写入 Header (需要编码为 bytes)
        f.write(header.encode('ascii'))
        
        # 2. 写入数据
        if num_points > 0:
            # 确保数据类型为 float32 (对应 SIZE 4 TYPE F)
            # tobytes() 会将数组转为纯二进制字节流
            f.write(points.astype(np.float32).tobytes())
            
    print(f"已保存(Binary): {file_path} (点数: {num_points})")
...
def main():
    parser = argparse.ArgumentParser(description="合并多雷达点云到 Base_link 并保留强度 (Binary Output)")
    parser.add_argument('--path', type=str, default='scene_data/bag_20251227_093155/9', help='场景路径，例如 scene_1')
    parser.add_argument('--extrinsic', type=str, default='utils/sensor_to_base.json', help='外参文件路径')
    ...
    print(f"输出目录: {os.path.join(scene_dir, 'lidar_point_cloud_0')} (Binary Format)")
